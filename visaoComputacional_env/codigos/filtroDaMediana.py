...
def filtroDaMediana(imagem):
    ...
    for l in range(numLinhas):
        for c in range(numColunas):
            ...
            listaOrdenada = subMatriz.flatten()
            listaOrdenada = sorted(listaOrdenada)
            
            imagem[l][c] = listaOrdenada[4]
    
    return imagem
        
imagem = cv2.imread('../images/fotografo.png', cv2.IMREAD_GRAYSCALE)
resultado = deepcopy(imagem)

for i in range(5):
    resultado = filtroDaMediana(resultado)
